...
from webtestagent.config.settings import configure_utf8_runtime, init_env, parse_bool
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--show-full-events",
        action="store_true",
        help="Print full stream events without truncation",
    )
    # session 持久化
    parser.add_argument(
        "--auto-load-session",
        action="store_true",
        help="Auto-load session state before run",
    )
    parser.add_argument(
        "--auto-save-session",
        action="store_true",
        help="Auto-save session state after run",
    )
    parser.add_argument(
        "--session-site-id", help="Override site ID for session storage"
    )
    parser.add_argument(
        "--session-account-id", help="Account identifier for multi-account sites"
    )
...
def main() -> None:
    ...
    session_config = SessionPersistenceConfig(
        ...
        site_id=args.session_site_id
        or os.getenv("SESSION_SITE_ID")
        or session_defaults.get("site_id"),
        account_id=args.session_account_id
        or os.getenv("SESSION_ACCOUNT_ID")
        or session_defaults.get("account_id"),
        storage_dir=(
            Path(args.session_dir)
            if args.session_dir
            else Path(session_defaults["storage_dir"])
            if session_defaults.get("storage_dir")
            else None
        ),
    )
    ...
    if prepared.session_state:
        ...
        print(
            f"[start] session: site={ss.site_id} account={ss.account_id or '_default'} "
