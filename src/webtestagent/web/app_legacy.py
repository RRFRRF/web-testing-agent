...
MAX_EVENTS = 500
MAX_BODY_SIZE = 1_048_576  # 1 MB 请求体上限
WEBAPP_API_KEY = os.getenv(
    "WEBAPP_API_KEY"
)  # 可选：设置后所有 /api/ 请求需带 ?key= 或 Header X-API-Key
...
def _list_runs() -> list[dict[str, Any]]:
    ...
    if OUTPUTS_DIR.exists():
        for run_dir in sorted(
            OUTPUTS_DIR.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True
        ):
            ...
            runs.append(
                {
                    ...
                    "completed_at": None,
                    "final_report": report_path.as_posix()
...
def _build_session_config(payload: dict) -> SessionPersistenceConfig:
    ...
    # S1: 校验 storage_dir 安全性
    storage_dir: Path | None = None
    raw_storage = sess.get("storage_dir")
    if raw_storage:
        p = Path(raw_storage)
        if p.is_absolute() or ".." in p.parts:
            raise ValueError(
                f"storage_dir must be a relative path without '..': {raw_storage}"
            )
        storage_dir = p
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def _check_api_key(self) -> bool:
        """校验 API Key。未配置 WEBAPP_API_KEY 时始终通过。"""
        if not WEBAPP_API_KEY:
            return True
        # 优先检查 Header，其次检查 query param
        header_key = self.headers.get("X-API-Key")
        if header_key == WEBAPP_API_KEY:
            return True
        query = parse_qs(urlparse(self.path).query)
        query_key = (query.get("key") or [None])[0]
        return query_key == WEBAPP_API_KEY
    ...
    def _read_json_body(self) -> dict[str, Any]:
        ...
        if length > MAX_BODY_SIZE:
            raise ValueError(
                f"Request body too large: {length} bytes (max {MAX_BODY_SIZE})"
            )
    ...
    def do_GET(self) -> None:
        ...
        if path.startswith("/api/runs/") and path.endswith("/events"):
            ...
            with session.condition:
                self._send_json(
                    {"events": list(session.events), "status": session.status}
                )
            return
