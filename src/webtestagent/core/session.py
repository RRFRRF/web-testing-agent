...
from webtestagent.config.settings import COOKIES_DIR
...
class ResolvedSessionState:
    ...
    resolved_by: str = "auto-none"  # explicit / auto-single / auto-none / auto-ambiguous
...
def _playwright_prefix() -> list[str]:
    """获取 playwright-cli 命令前缀。"""
    cli = shutil.which("playwright-cli")
    if cli:
        return [cli]

    cli_cmd = shutil.which("playwright-cli.cmd")
    if cli_cmd:
        return [cli_cmd]
